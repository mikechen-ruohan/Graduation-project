...
class YoloPredictor(BasePredictor, QObject):
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None):
        super(YoloPredictor, self).__init__() #对BasePredictor和QObject两个父类初始化
        QObject.__init__(self) #是不是多余了？
        ...
        self.count_of_car = None
        self.count_of_van= None
        self.count_of_truck = None
        self.class_num = None
    ...
    def run(self):
        ...
        # 折线图数据初始化
        global x_axis_time_graph, y_axis_count_graph, y_axis_car_graph, y_axis_van_graph, y_axis_truck_graph #新改动，增加全局变量
        x_axis_time_graph = []
        y_axis_count_graph = []
        y_axis_car_graph = []
        y_axis_truck_graph =[]
    ...
    def res_address(self, img_res, result, height, width, model, out):
            ...
            # 如果有识别的
                ...
                #新增：统计car van truck的数量
                car_ids=np.array([])
                van_ids=np.array([])
                truck_ids=np.array([])
                for box in result.boxes:
                    if box.cls==0:
                        car_ids = np.append(car_ids, box.id.cpu().numpy().astype(int))
                    if box.cls==1:
                        van_ids = np.append(van_ids, box.id.cpu().numpy().astype(int))
                    if box.cls==2:
                        truck_ids = np.append(truck_ids, box.id.cpu().numpy().astype(int))
                # id 、位置、目标总数
                ...
                self.count_of_car = len(car_ids) #新增：私家车总数
                self.count_of_van = len(van_ids) #新增：面包车总数
                self.count_of_truck = len(truck_ids)#新增：卡车总数
                # 轨迹绘制部分 @@@@@@@@@@@@@@@@@@@@@@@@@@@@
                if self.show_trace:
            ...
            if new_time not in x_axis_time_graph:  # 防止同一秒写入
                ...
                y_axis_car_graph.append(self.count_of_car)
                y_axis_van_graph.append(self.count_of_van)
                y_axis_truck_graph.append(self.count_of_truck)

...
from user_management_main import UserWindow
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        UIFuncitons.shadow_style(self, self.Target_QF, QColor(255, 255, 255))
        UIFuncitons.shadow_style(self, self.Fps_QF, QColor(255, 255, 255))
        UIFuncitons.shadow_style(self, self.Model_QF, QColor(255, 255, 255))

        # 设置 为 -- （好看）
        self.Class_num.setText('未检测')
        self.Target_num.setText('未检测')
        self.fps_label.setText('未检测')
...
#############################################################################################################################
        ...
        self.show_status('尊敬的管理员您好，欢迎使用智慧车流识别分析系统！')
        self.login=LoginWindow(self)
        self.hide()
    # 实例化YOLO
    def yolo_init(self):
        # Yolo-v8 thread
        self.yolo_predict = YoloPredictor()   #classes.yolo里面一个类名叫做YoloPredictor，把他实例化名为yolo_predict      实例化yolo检测
        self.select_model = self.model_box.currentText() #初始化的对象self继承了PYQT里面的东西，PYQT里有个类有这个方法，表示下拉文本框，这是选择模型的按钮
        self.yolo_thread = QThread() #新建一个PYQT线程

        # 显示预测视频（左，右）
        self.yolo_predict.yolo2main_trail_img.connect(lambda x: self.show_image(x, self.pre_video)) #轨迹图像信号
        self.yolo_predict.yolo2main_box_img.connect(lambda x: self.show_image(x, self.res_video)) #标签与锚框的信号
...
#       self.src_rtsp_button.clicked.connect(self.rtsp_seletction) # RTPS  #（main_window.py中235和1835行）
...
#        self.src_web_button.clicked.connect(self.web_back_end)   # 后端开启与关闭  #main_window.py中
        self.user_management_button.clicked.connect(self.user_management)#绑定用户管理
    ...
    def run_or_continue(self):
        ...
        if self.yolo_predict.new_model_name == '' or self.yolo_predict.new_model_name == None:
            DialogOver(parent=self, text="请确认当前模型的可靠性", title="运行失败", flags="danger")
            self.run_button.setChecked(False)
            return
        # 检测输入源
    ...
    def user_management(self):
        self.user_management_window = UserWindow()
    ...
    def show_traffic_graph(self):
        ...
        if not self.run_button.isChecked():
            DialogOver(parent=self, text="请先通过本地或者实时视频流方式进行检测方可启动流量分析", title="开启失败", flags="danger")
            return

        # 避免重复开启
        if self.is_draw_thread:
            DialogOver(parent=self, text="流量图已经成功开启", title="不能重复开启", flags="danger")
            return

...
class UserWindow(MouseEvent):
    ...
    def __init__(self, payPage=None):
        ...
        self.ui = Ui_manage() #实例化UI
        self.ui.setupUi(self) #初始化ui
        self.setStyleSheet(readQss('./gui_style/main.qss')) #读取布局样式
        self.setWindowIcon(QIcon("logo.ico")) #读取图标
        #self.login = LoginWindow(self) #实例化登录界面
    ...
    def changePage(self, page: int):
        ...
        btn = [self.ui.manage_btn, self.ui.setting_btn]
        btn_name = ['manage', 'setting']
        if self.extend:
            self.extend_show()
    ...
    def extend_show(self, show: bool = True):
        ...
        menu_btn = [self.ui.extend_btn, self.ui.manage_btn, self.ui.setting_btn, self.ui.exit_btn]
        #menu_name = ['extend', 'manage', 'setting', 'about', 'exit']
        menu_name = ['extend', 'manage', 'setting', 'exit']
        if show:
            menu_text = ['扩展', '管理', '设置', '退出']
